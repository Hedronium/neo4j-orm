...
class Core {
		...
		createNode(nodes,data) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				this.querynode.push(node[i]);
				i++;

			}

			for (var j = 0; i < data.length; ) {

			}

			if (i !== j) {

				this.queryError.push('Error: number of objects and number of nodes must be same!');
			}
		...
		if (this.queryType == 0) {
			...
			this.querystring.concat('CREATE ');
		}

		this.querydata.forEach(function(key,value){
				let data = key+'=>'+value;
				console.log(data);
